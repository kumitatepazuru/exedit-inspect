"""Locate the モザイク (mosaic) filter's FILTER_DLL entries in exedit.auf and
dump their full structs: func_proc/func_init addresses plus the trackbar and
checkbox definitions (names, defaults, ranges), so the disassembly that
follows can be tied back to the parameters exposed in the AviUtl UI.

Like ぼかし/発光 (see inspect/blur, inspect/glow), モザイク is registered TWICE -
once as an object effect (flag & 0x20) and once as a frame(video) filter -
and both registrations share the same func_proc. This script prints how many
registrations it finds so that stays checked instead of assumed.

Run via main.py:
    uv run main.py inspect/mozaic/find_mozaic_addr.py
    uv run main.py inspect/mozaic/find_mozaic_addr.py --target-name "ノイズ"
"""
...
def run(dll_path: str, headers: list[str], argv: list[str] | None = None) -> None:
    ...
    while True:
        ...
        if plausible(name_ptr) and read_cstring(name_ptr) == args.target_name:
            ...
            found.append({
                ...
                "func_wndproc": read_dword(entry_va + OFF_FUNC_WNDPROC),
                "track_n": track_n,
                "check_n": check_n,
    ...
    for f in found:
        ...
        print(f"  func_proc = 0x{f['func_proc']:08x}   func_init = 0x{f['func_init']:08x}"
              f"   func_WndProc = 0x{f['func_wndproc']:08x}")
        ...
        for i in range(f["check_n"]):
            # a negative check_default means the entry is a button, not a checkbox
            kind = "button" if (f["check_defaults"][i] or 0) < 0 else "checkbox"
            print(f"    check[{i}] name={f['check_names'][i]!r} "
                  f"default={f['check_defaults'][i]} ({kind})")

    print("\n" + "=" * 66)
    print(f"{len(found)} registration(s) found for {args.target_name!r}.")
    procs = {f["func_proc"] for f in found}
    print(f"distinct func_proc addresses: {', '.join('0x%08x' % p for p in sorted(procs))}"
