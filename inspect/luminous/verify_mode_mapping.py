...
checkbox, because an object's canvas has to grow when the luminous spills past
its bounding box - is the object-effect path.

Run via main.py:
    uv run main.py inspect/luminous/verify_mode_mapping.py
"""
...
ANNOTATIONS = {
    ...
    0x1005329B: "ecx = *(fpip+0xB0) = luminous accumulator buffer",
...
def run(dll_path: str, headers: list[str], argv: list[str] | None = None) -> None:
    ...
    print(
        ...
        "      luminous src : fpip->ycp_temp (+8)          accum: *(fpip+0xB0), 6 bytes/pixel\n"
        ...
        "      luminous src : fpip->ycp_temp (+8)          accum: ycp_edit itself, or\n"
        ...
        "need to grow when the luminous spills outside it (hence the size-fixed checkbox + the\n"
