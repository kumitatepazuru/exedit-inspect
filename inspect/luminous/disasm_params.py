...
decompile_luminous.py's angr-based decompilation renders most of this as
...
point at) is what actually pins down:
  ...
  * the geometric progression of blur radii across the fixed 6-pass loop
    ...
    decompile_luminous.py's FP_HELPERS section - not just angr's inferred
...
Run via main.py:
    uv run main.py inspect/luminous/disasm_params.py
"""
...
# the luminous *tint color* setup, unrelated to 拡散速度. Included here so the
# two are not confused (an earlier pass at this analysis conflated them).
luminous_COLOR_SETUP = (0x1006fed0, 0x60)
...
# luminous-specific (they write into a completely different global range,
# 0x101e42f4.., than luminous's own 0x101b1fe0.. globals, and the multithread
...
def run(dll_path: str, headers: list[str], argv: list[str] | None = None) -> None:
    dump_all(PEImage(dll_path), {
        ...
        "sub_1006fed0 (光色の設定 -> Y/Cb/Cr, NOT 拡散速度)": luminous_COLOR_SETUP,
